        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                download_task = asyncio.create_task(
                    page.wait_for_event("download", timeout=20000)
                )
                toast_task = asyncio.create_task(
                    page.wait_for_selector(".toast-message", timeout=20000)
                )
                await botao.click()
                ...
                if toast_task in done:
                    try:
                        ...
                        mensagem = (await toast.inner_text() or "").strip()
                    except PlaywrightTimeoutError:
                        mensagem = None
                    except Exception as exc:
                        print(f"[MEGASOFT] Falha ao capturar toast: {exc}")
                        mensagem = None

                    if mensagem:
                        return {"ok": False, "info": mensagem, "path": None, "url": None}

                    mensagem_timeout = await _wait_for_toast_message(page, timeout=8000)
                    if mensagem_timeout:
                        return {
                            "ok": False,
                            "info": mensagem_timeout,
                            "path": None,
                            "url": None,
                        }
                    ...
                    return {
                        "ok": False,
                        "info": "Não foi possível gerar a certidão (toast não disponível).",
                        "path": None,
                        "url": None,
                    }
