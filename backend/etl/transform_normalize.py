...
def _transform_processos(section: Dict[str, Iterable[Dict[str, Any]]], contract: ConfigContract) -> List[NormalizedRow]:
    ...
    for logical_table, rows in section.items():
        ...
        for index, row in enumerate(rows, start=1):
            ...
            situacao_raw = (
                mapped.get("SITUACAO")
                or mapped.get("STATUS_PADRAO")
                or "PENDENTE"
            )
            situacao = _normalize_enum(
                situacao_raw,
                enum_situacao,
                "situacao",
                tipo,
            ...
            data_solicitacao = _safe_parse_date(
                mapped.get("DATA_SOLICITACAO"), "data_solicitacao", tipo
            )
            if data_solicitacao is None:
                raise TransformError(
                    f"Data de solicitação obrigatória em processos {tipo}"
                )
            ...
            if logical_table == "diversos":
                payload["operacao"] = _normalize_enum(
                    mapped.get("OPERACAO"),
                    enum_operacao,
                    "operacao",
                    tipo,
            ...
            if logical_table == "funcionamento":
                payload["alvara"] = _normalize_enum(
                    mapped.get("ALVARA"),
                    enum_alvara,
                    "alvara",
                    tipo,
            ...
            if logical_table == "sanitario":
                payload["servico"] = _normalize_enum(
                    mapped.get("SERVICO"),
                    enum_servico,
                    "servico",
                    tipo,
                ...
                payload["notificacao"] = _normalize_enum(
                    mapped.get("NOTIFICACAO"),
                    enum_notificacao,
                    "notificacao",
                    tipo,
                    required=False,
                ...
                payload["data_val"] = _safe_parse_date(
                    mapped.get("DATA_VAL"), "data_val", tipo
                )
            row_number = _row_number(row, index)
            normalized.append(
                NormalizedRow(
