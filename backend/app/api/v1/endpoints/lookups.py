...
def _map_cnae_item(item: Any) -> dict[str, str] | None:
...
async def _fetch_receitaws(cnpj: str) -> dict[str, Any]:
...
async def lookup_receitaws(
    ...
    try:
        data = await _fetch_receitaws(digits)
    ...
    cnaes_principal = [entry for entry in (_map_cnae_item(item) for item in principal) if entry]
    cnaes_secundarios = [entry for entry in (_map_cnae_item(item) for item in secundarios) if entry]
