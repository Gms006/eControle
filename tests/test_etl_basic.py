...
def test_placeholder_process_defaults(contract):
    raw = sample_raw_data()
    normalized = transform(raw, contract)
    placeholder_row = next(
        row
        for row in normalized["processos"]
        if row.row_number == 5
    )
    assert placeholder_row.payload["situacao"] == "PENDENTE"
